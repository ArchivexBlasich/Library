...
class Book {
  ...
  info() {
    return `${capitalizeWords(this.title)} by ${capitalizeWords(
      this.author
...
function deleteBook(e) {
  if (
    ...
    e.target.classList.value !== "delete-btn-book"
  )
    return;
...
function toggleRead(e) {
  if (
    e.target.tagName !== "BUTTON" ||
    e.target.classList.value !== "read-btn-book"
  )
    return;
  let bookContainer = e.target.parentNode.parentNode;
  let id = bookContainer.dataset.bookid;
  ...
  if (indexBook !== -1) {
    ...
    let para = document.querySelector(
      `article[data-bookid="${id}"]  > div:nth-of-type(1) > p`
    );
